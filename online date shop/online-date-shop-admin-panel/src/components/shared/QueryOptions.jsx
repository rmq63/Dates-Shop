...
function QueryOptions({ query, setQuery, disabledSearch }) {
  return (
    <div className='flex flex-col items-center justify-between space-x-0 space-y-2 sm:flex-row sm:space-x-2 sm:space-y-0'>
      <Input className='space-x-4' onChange={(e) => setQuery((prevState) => ({ ...prevState, search: e.target.value }))} placeholder='Start type here to Search...' prefix={<SearchOutlined />} disabled={disabledSearch} value={query.search} size='large' allowClear />

      <Select className='w-full sm:w-[240px]' onChange={(value) => setQuery((prevState) => ({ ...prevState, rows: value }))} placeholder='-- show rows --' defaultValue={query.rows} size='large'>
      ...
      <Select className='w-full sm:w-[240px]' onChange={(value) => setQuery((prevState) => ({ ...prevState, sort: value }))} placeholder='-- select type to sort --' defaultValue={query.sort} size='large'>
